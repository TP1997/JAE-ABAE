...
class JAE_ABAE(torch.nn.Module):
    def __init__(self, args, word_embeddings, 
        ...
        self.word_embedding = torch.nn.Embedding.from_pretrained(word_embeddings,
                                                                  freeze=True)
        # Corpus-specific attention mechanisms.
        self.attention_1 = SelfAttention(self.emb_dim)
        self.attention_2 = SelfAttention(self.emb_dim)
    ...
    def document_topic_dist(self, text_embeddings, branch: str):
        ...
        if branch=='p1':
            attention_weights = self.attention_1(text_embeddings)
            ...
            attention_weights = self.attention_2(text_embeddings)
            doc_emb = torch.matmul(attention_weights.unsqueeze(1), text_embeddings).squeeze()
    ...
    def forward(self, text_embeddings_idx_r1, text_embeddings_idx_r2, negative_samples_idx_r1, negative_samples_idx_r2):
        ...
        _, p_p1, doc_emb_1 = self.document_topic_dist(text_embeddings_r1, branch='p1')
        _, p_s1, _ = self.document_topic_dist(text_embeddings_r1, branch='s1')
        _, p_p2, doc_emb_2 = self.document_topic_dist(text_embeddings_r2, branch='p2')
        _, p_s2, _ = self.document_topic_dist(text_embeddings_r2, branch='s2')
