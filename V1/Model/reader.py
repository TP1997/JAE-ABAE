...
class TrainingDataset(Dataset):
    def __init__(self, data_conf, vocabulary, pad_idx):
        self.maxlen = data_conf['maxlen']
        self.size_R1 = data_conf['R1']['size']
        self.size_R2 = data_conf['R2']['size']
        self.size = max(self.size_R1, self.size_R2)
        self.pad_idx = pad_idx
        
        self.data_R1 = []
        with open(data_conf['R1']['train_file_text'], 'r', encoding='utf-8') as f:
            for line in f:
                line = line.strip().split()
                # Get vocabulary indices corresponding to word.
                vocab_idxs = [vocabulary[w] for w in line]
                self.data_R1.append(vocab_idxs)
                
        self.data_R2 = []
        with open(data_conf['R2']['train_file_text'], 'r', encoding='utf-8') as f:
            for line in f:
                line = line.strip().split()
                # Get vocabulary indices corresponding to word.
                vocab_idxs = [vocabulary[w] for w in line]
                self.data_R2.append(vocab_idxs)                
                
    def __len__(self):
        return self.size
        
    def __getitem__(self, idx):
        item_R1 = self.data_R1[np.mod(idx,self.size_R1)]
        if len(item_R1) < self.maxlen:
                item_R1.extend([self.pad_idx] * (self.maxlen - len(item_R1)))
                
        item_R2 = self.data_R2[np.mod(idx,self.size_R2)]
        if len(item_R2) < self.maxlen:
                item_R2.extend([self.pad_idx] * (self.maxlen - len(item_R2)))
                
        return np.asarray(item_R1, dtype=np.int64), np.asarray(item_R2, dtype=np.int64)
    
    def get_negative_samples(self, negative_samples, batch_size):
        pass
...
def save_topics(topics: list, savefile, readable=True):
    if readable:
